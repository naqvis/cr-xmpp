...
module XMPP::Stanza
  ...
  class DelayMessage < MsgExtension
    ...
    def self.new(node : XML::Node)
      ...
      raise "Invalid node(#{ns} #{node.name}) expecting: #{@@xml_name}" unless (node.namespace.try &.href == @@xml_name.space) &&
  ...
  Registry.map_extension(PacketType::Message, XMLName.new("urn:xmpp:delay", "delay"), DelayMessage)
end
